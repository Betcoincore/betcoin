...
// ***TODO*** ScanHash is not yet used in Bet
//
    ...
    try {
        ...
        while (true) {
            ...
            */
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BetMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("BetMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
