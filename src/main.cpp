...
const string strMessageMagic = "Bet Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if(nHeight <= 1 ){
		...
		int ex = (nHeight - 1500000) / 500000;
		double mul = std::pow(0.75, 1 + ex) * 50;
		nSubsidy = mul * COIN;
	}
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
	...
	static int64_t nLastCoinStakeSearchTime = GetAdjustedTime(); // startup timestamp//myfix 1 stake for per 60s
	...
	if (nSearchTime > nLastCoinStakeSearchTime+60)//myfix
	{
		//printf("\n1=%lld\n",nSearchTime);
