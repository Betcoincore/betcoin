...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBetAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBetAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBetAmount = strAnonymizeBetAmount.remove(strAnonymizeBetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBetAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBetAmount*COIN) nMaxToAnonymize = nAnonymizeBetAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBetAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBetAmount));
        strAnonymizeBetAmount = strAnonymizeBetAmount.remove(strAnonymizeBetAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBetAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
