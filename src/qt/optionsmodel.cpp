...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBetAmount"))
        settings.setValue("nAnonymizeBetAmount", 1000);
    nAnonymizeBetAmount = settings.value("nAnonymizeBetAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBetAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBetAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBetAmount:
            return QVariant(nAnonymizeBetAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBetAmount:
            nAnonymizeBetAmount = value.toInt();
            settings.setValue("nAnonymizeBetAmount", nAnonymizeBetAmount);
            emit AnonymizeBetAmountChanged(nAnonymizeBetAmount);
            break;
        default:
            break;
        }
